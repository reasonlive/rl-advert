...
$count = $mysqli->query("SELECT `id` FROM `tb_black_sites`")->num_rows;
...
if($option=="addbl" && count($_POST)>0) {
	...
	$sql_bl = $mysqli->query("SELECT `domen` FROM `tb_black_sites` WHERE `domen`='$domen'");
	...
	if($url==false || $cause==false){
		...
		$mysqli->query("INSERT INTO `tb_black_sites` (`url`,`domen`,`cause`,`date`,`ip`) VALUES('$url','$domen','$cause','".time()."','".@$_SERVER['REMOTE_ADDR']."')") or die($mysqli->error);
...
if($option=="edit") {
	...
	$sql_e = $mysqli->query("SELECT * FROM `tb_black_sites` WHERE `id`='$id'");
	if($sql_e->num_rows>0) {
		$row_e = $sql_e->fetch_array();
		...
		if(count($_POST)>0) {
			...
			if($url==false || $cause==false){
				...
				$status_edit="OK";
				$mysqli->query("UPDATE `tb_black_sites` SET `url`='$url',`domen`='$domen',`cause`='$cause',`date`='".time()."',`ip`='".@$_SERVER['REMOTE_ADDR']."'  WHERE `id`='$id'") or die($mysqli->error);
...
if($option=="dell") {
	...
	$sql_del = $mysqli->query("SELECT `id` FROM `tb_black_sites` WHERE `id`='$id'");
	if($sql_del->num_rows>0){
		$mysqli->query("DELETE FROM `tb_black_sites` WHERE `id`='$id'") or die($mysqli->error);
...
$sql = $mysqli->query("SELECT * FROM `tb_black_sites` ORDER BY `id` DESC LIMIT  $start_pos,$perpage");
if($sql->num_rows>0) {
	while ($row=$sql->fetch_array()) {
		...
		echo '<td width="20.6%">'.$row["cause"].'</td>';
