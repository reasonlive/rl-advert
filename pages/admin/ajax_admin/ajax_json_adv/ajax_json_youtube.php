...
if($option == "PlayPause") {
	$sql = $mysqli->query("SELECT * FROM `tb_ads_youtube` WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
	if($sql->num_rows>0) {
		$row = $sql->fetch_assoc();
		$status = $row["status"];
		$nolimit = $row["nolimit"];
		...
		if($status == 0) {
			...
			if($nolimit > 0) {
				...
				$mysqli->query("UPDATE `tb_ads_youtube` SET `status`='2' WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);

				$status_text = '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'youtube\', \'PlayPause\');"></span>';
				$message_text = "";
			...
			if($nolimit > 0) {
				...
				$mysqli->query("UPDATE `tb_ads_youtube` SET `status`='1' WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);

				$status_text = '<span class="adv-pause" title="Приостановить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'youtube\', \'PlayPause\');"></span>';
				$message_text = "";
				$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "OK", "status" => iconv("CP1251", "UTF-8", $status_text), "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
				exit($js_result);
			}
			...
			$mysqli->query("UPDATE `tb_ads_youtube` SET `status`='2', `user_lock`='', `msg_lock`='' WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
	...
	if($sql->num_rows>0) {
		...
		if($status==4) {
			...
			$message_text.= '<div align="center" style="float:left; padding-left:5px; padding-top:6px;"><span onClick="Lock(\''.$row["id"].'\', \'youtube\', \'Lock\');" class="sub-red" style="float:none;" title="Заблокировать">Заблокировать</span></div>';
	...
	if($sql->num_rows>0) {
		...
		if($status==4) {
			...
			$mysqli->query("UPDATE `tb_ads_youtube` SET `status`='4', `user_lock`='$username', `msg_lock`='$msg_lock' WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
		...
		$message_text.= '<div class="box-modal" id="ModalClaimsSurf" style="text-align:justify; width:900px;">';
			...
			$message_text.= '<div class="box-modal-content" style="margin:1px; padding:5px 3px; font-size:11px;">';
				...
				$message_text.= '<div id="table-content" style="overflow:auto;">';
					$message_text.= '<table class="tables" style="margin:0 auto; padding:0;">';
					$sql = $mysqli->query("SELECT * FROM `tb_ads_claims` WHERE `ident`='$id' AND `type`='youtube' ORDER BY `id` DESC");
					if($sql->num_rows>0) {
						while ($row = $sql->fetch_assoc()) {
							$message_text.= '<tr id="claims-'.$row["id"].'" align="center">';
								...
								$message_text.= '<td width="20"><span class="clear-claims" title="Удалить жалобу" onClick="DelClaims('.$row["id"].', \'youtube\', \'DelClaims\');"></span></td>';
							$message_text.= '</tr>';
						}
					}else{
	...
	$sql = $mysqli->query("SELECT `ident` FROM `tb_ads_claims` WHERE `id`='$id' AND `type`='youtube'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
	if($sql->num_rows>0) {
		$row = $sql->fetch_assoc();
		$ident = $row["ident"];

		$mysqli->query("DELETE FROM `tb_ads_claims` WHERE `id`='$id' AND `type`='youtube'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
		$mysqli->query("UPDATE `tb_ads_youtube` SET `claims`=`claims`-'1' WHERE `id`='$ident'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);

		$count_claims = $mysqli->query("SELECT `claims` FROM `tb_ads_youtube` WHERE `id`='$ident'");
	...
	if($sql->num_rows>0) {
		...
		$mysqli->query("UPDATE `tb_ads_youtube` SET `claims`='0' WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
		$mysqli->query("DELETE FROM `tb_ads_claims` WHERE `ident`='$id' AND `type`='youtube'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
	...
	$sql_check = $mysqli->query("SELECT `id` FROM `tb_ads_youtube` WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
	if($sql_check->num_rows>0) {
		$mysqli->query("DELETE FROM `tb_ads_youtube` WHERE `id`='$id'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $mysqli->error))) : $mysqli->error);
