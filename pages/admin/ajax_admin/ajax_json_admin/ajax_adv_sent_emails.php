...
if($option == "Delete") {
	$sql_check = $mysqli->query("SELECT `id` FROM `tb_ads_emails` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
	if($sql_check->num_rows>0) {
		$mysqli->query("DELETE FROM `tb_ads_emails` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
	...
	$sql = $mysqli->query("SELECT * FROM `tb_ads_emails` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
	if($sql->num_rows>0) {
		$row = $sql->fetch_assoc();
		$status = $row["status"];
		$sent = $row["sent"];
		...
		if($status == 0 | ($sent == 0 && $nosent == 0)) {
			...
			$mysqli->query("UPDATE `tb_ads_emails` SET `sent`='0',`nosent`='0' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
	...
	$sql_id = $mysqli->query("SELECT * FROM `tb_ads_emails` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
	if($sql_id->num_rows>0) {
		...
		$mysqli->query("UPDATE `tb_ads_emails` SET `status`='1', `date`='".time()."' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
	...
	if($sql->num_rows>0) {
		...
		if($status == 0) {
			...
			$mysqli->query("UPDATE `tb_ads_emails` SET `status`='2', `date`='".time()."' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));

			$result_text = "OK"; $message_text = '<span class="adv-play" title="Запустить рассылку на e-mail" onClick="PlayPause('.$row["id"].', \'sent_emails\', \'PlayPause\');"></span>';
			exit(my_json_encode($ajax_json, $result_text, $message_text));

		}elseif($status == 2) {
			$mysqli->query("UPDATE `tb_ads_emails` SET `status`='1', `date`='".time()."' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));

			$result_text = "OK"; $message_text = '<span class="adv-pause" title="Приостановить рассылку на e-mail" onClick="PlayPause('.$row["id"].', \'sent_emails\', \'PlayPause\');"></span>';
			exit(my_json_encode($ajax_json, $result_text, $message_text));
	...
	if($sql->num_rows>0) {
		...
		$message_text.= '<div align="center"><span id="Save" onClick="SaveAds(\''.$id.'\', \'sent_emails\', \'Save\');" class="sub-blue160" style="float:none; width:160px;">Сохранить изменения</span></div>';
	...
	if($subject == false) {
		...
		if($sql_check->num_rows>0) {
			...
			$mysqli->query("UPDATE `tb_ads_emails` SET `date`='".time()."',`subject`='$subject',`message`='$message' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
