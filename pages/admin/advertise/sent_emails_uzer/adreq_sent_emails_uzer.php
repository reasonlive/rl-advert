...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_emails_uzer` WHERE `status`='0'");
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="center" width="35">';
				echo '<div id="playpauseimg'.$row["id"].'">';
					echo '<span class="adv-play" title="Запустить рассылку на e-mail" onClick="AdsReq('.$row["id"].', \'sent_emails_uzer\', \'Start\');"></span>';
				echo '</div>';
			echo '</td>';
