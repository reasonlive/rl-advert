...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Неоплаченные заказы псевдо-динамической рекламы</b></h3>';
...
if(isset($_GET["id"])) {
	...
	if($option=="add") {
		$sql = $mysqli->query("SELECT * FROM `tb_ads_txt` WHERE `id`='$id' AND `status`='0'");
		if($sql->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_txt` SET `status`='1', `date`='".time()."', `date_end`=`plan`*'".(24*60*60)."'+'".time()."' WHERE `id`='$id'") or die($mysqli->error);
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_txt` WHERE `status`='0' ORDER BY `id` ASC");
if($sql->num_rows>0) {
	while ($row = $sql->fetch_array(l)) {
		echo '<tr align="center">';
