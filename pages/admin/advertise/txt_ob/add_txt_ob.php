...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Добавление текстового объявления</b></h3>';
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	if($sql_id->num_rows>0) {
		...
		$mysqli->query("UPDATE `tb_ads_txt` SET `status`='1', `date`='".time()."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_txt` SET `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `ip`='$laip', `money`='0' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
