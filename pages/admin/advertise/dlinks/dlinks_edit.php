...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Редактирование динамической ссылки</b></h1>';
...
$mysqli->query("UPDATE `tb_ads_dlink` SET `status`='3', `date`='".time()."' WHERE `status`>'0' AND `status`<'3' AND ( (`totals`<'1' AND `nolimit`='0') OR ( `nolimit`>'0' AND `nolimit`<='".time()."') )") or die($mysqli->error);
...
if(isset($_POST["search"]) && isset($_POST["metode"])) {
	$metode = isset($_POST["metode"]) ? $mysqli->real_escape_string($_POST["metode"]) : false;
	$search = isset($_POST["search"]) ? $mysqli->real_escape_string($_POST["search"]) : false;
	$operator = isset($_POST["operator"]) ? intval($mysqli->real_escape_string($_POST["operator"])) : 0;
...
if(isset($_GET["search"]) && isset($_GET["metode"])) {
	$metode = isset($_GET["metode"]) ? $mysqli->real_escape_string($_GET["metode"]) : false;
	$search = isset($_GET["search"]) ? $mysqli->real_escape_string($_GET["search"]) : false;
	$operator = isset($_GET["operator"]) ? intval($mysqli->real_escape_string($_GET["operator"])) : false;
...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_dlink` WHERE `status`>'0' AND `type_serf`!='10' $WHERE_ADD");
...
if(isset($_POST["id"])) {
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		$sql_totals = $mysqli->query("SELECT `plan`,`totals` FROM `tb_ads_dlink` WHERE `id`='$id' AND `type_serf`!='10'");
...
if(isset($_GET["id"])) {
	...
	if ($option=="edit"){
		$sql = $mysqli->query("SELECT * FROM `tb_ads_dlink` WHERE `id`='$id' AND `status`>'0' AND `type_serf`!='10'");
		if($sql->num_rows>0) {
			$row = $sql->fetch_array();
			...
			echo '<tr>';
				...
				if($row["type_serf"]==1) {
					echo '<td><input type="hidden" name="type_serf" value="'.$row["type_serf"].'">Динамический серфинг</td>';
					...
					echo '<td><input type="hidden" name="type_serf" value="'.$row["type_serf"].'">Динамический серфинг - VIP</td>';
				...
				echo '<tr>';
					...
					echo '<td>';
						echo '<select name="revisit">';
							...
							echo '<option value="2" '.("".$row["revisit"]."" == 2 ? 'selected="selected"' : false).'>1 раз в месяц</option>';
						echo '</select>';
					echo '</td>';
				echo '</tr>';
				echo '<tr>';
...
$sql_ads = $mysqli->query("SELECT * FROM `tb_ads_dlink` WHERE `status`>'0' AND `type_serf`!='10' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die($mysqli->error);
...
$time_s=time()-24*60*60*14;
$pauze_all = $mysqli->query("SELECT * FROM `tb_ads_dlink` WHERE `status`='2' && `data_pauza`<'".$time_s."' order by date desc")->num_rows;

echo '<table class="tables" style="margin:1px auto;">';
echo '<thead><tr align="center">';
	echo '<th>Запустить</th>';
	echo '<th>Количество рекламы на паузе</th>';
echo '</tr></thead>';
echo '<tbody>';
		echo '<tr align="center">';
		echo '<td align="center" width="30" class="noborder1">';
			echo '<div id="playpauseimg">';
				if($status=2) {
					echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="play_pause('.$row["id"].', \'dlinkplau\');"></span>';
				}
					echo '</div>';
		echo '</td>';

		echo '<td>'.number_format($pauze_all,0,".","`").' шт.</td>';
