...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Добавление динамической ссылки</b></h1>';
...
if(count($_POST)>0) {
	...
	$plan = ( isset($_POST["plan"]) && preg_match("|^[\d]{1,11}$|", trim($_POST["plan"])) && intval(limpiarez(trim($_POST["plan"]))) >= $dlink_min_hits ) ? intval(limpiarez(trim($_POST["plan"]))) : false;
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
	...
	}elseif($nolimit==0 && $plan<$dlink_min_hits) {
		...
		$mysqli->query("INSERT INTO `tb_ads_dlink` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`type_serf`,`date`,`wmid`,`username`,`geo_targ`,`content`,`active`,`revisit`,`color`,`timer`,`nolimit`,`limit_d`,`limit_h`,`limit_d_now`,`limit_h_now`,`new_users`,`unic_ip`,`no_ref`,`sex_adv`,`to_ref`,`url`,`title`,`description`,`urlbanner_load`,`plan`,`totals`,`ip`,`money`) 
		VALUES('1','".session_id()."','0','$method_pay','$type_serf','".time()."','$wmid_user','$username','$country','$content','$active','$revisit','$color','$timer','$nolimitdate','$limit_d','$limit_h','$limit_d','$limit_h','$new_users','$unic_ip','$no_ref','$sex_adv','$to_ref','$url','$title','$description','$urlbanner_load','$plan','$plan','$laip','0')") or die($mysqli->error);

		if($type_serf==3) {
			$mysqli->query("UPDATE `tb_users` SET `vip_serf`=`vip_serf`+'200' WHERE `username`='$username'") or die($mysqli->error);
		...
		echo '<tr>';
			...
			echo '<td><div id="bl6">';
				echo '<select id="revisit" name="revisit" onChange="PlanChange();" onClick="PlanChange();">';
					...
					echo '<option value="2">1 раз в месяц</option>';
				echo '</select>';
