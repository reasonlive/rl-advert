...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Просмотр жалоб на рекламу в серфинге</b></h1>';
...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_dlink` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='serf') ");
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_dlink` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='serf') ORDER BY `id` DESC LIMIT $start_pos,$perpage ");
if($sql->num_rows>0) {
	while ($row = $sql->fetch_assoc()) {
		...
		echo '<td align="center" width="30" class="noborder1" style="border-right:solid 1px #DDDDDD;">';
			echo '<div id="playpauseimg'.$row["id"].'">';
				if($row["status"]=="0") {
					...
					echo '<span class="adv-pause" title="Приостановить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'surfing\', \'PlayPause\');"></span>';
				}elseif($row["status"]=="2") {
					echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'surfing\', \'PlayPause\');"></span>';
				}elseif($row["status"]=="3") {
					echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'surfing\', \'PlayPause\');"></span>';
				}elseif($row["status"]=="4") {
					echo '<span class="adv-block" title="Рекламная площадка заблокирована" onClick="PlayPause('.$row["id"].', \'surfing\', \'PlayPause\', \'1\');"></span>';
				}
			echo '</div>';
		echo '</td>';
		...
		echo '<td width="80">';
			echo '<span class="adv-dell" title="Удалить рекламную площадку" onClick="DelAdv('.$row["id"].', \'surfing\', \'Delete\');"></span>';
			echo '<span class="clear-claims" title="Удалить все жалобы" onClick="DelAllClaims(\''.$row["id"].'\', \'surfing\', \'DelAllClaims\');"></span>';
			echo '<span class="view-claims" title="Просмотр жалоб" onClick="ViewClaims(\''.$row["id"].'\', \'surfing\', \'ViewClaims\');"></span>';
		echo '</td>';
