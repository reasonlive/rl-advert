...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_emails` WHERE `status`='1'");
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_emails` WHERE `status`>'0' ORDER BY `id` DESC LIMIT $start_pos, $perpage");
if($sql->num_rows>0) {
	while ($row = $sql->fetch_assoc()) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="center" width="35">';
				echo '<div id="playpauseimg'.$row["id"].'">';
					if($row["status"]=="0") {
						...
						echo '<span class="adv-pause" title="Приостановить рассылку на e-mail" onClick="PlayPause('.$row["id"].', \'sent_emails\', \'PlayPause\');"></span>';
					}elseif($row["status"]=="2") {
						echo '<span class="adv-play" title="Запустить рассылку на e-mail" onClick="PlayPause('.$row["id"].', \'sent_emails\', \'PlayPause\');"></span>';
					}elseif($row["status"]=="3") {
						echo '<span class="adv-play" title="Запустить рассылку на e-mail" onClick="alert_nostart();"></span>';
					}elseif($row["status"]=="4") {
			...
			echo '<td align="left">';
				...
				echo '<span class="adv-dell" title="Удалить рассылку" onClick="AdsReq('.$row["id"].', \'sent_emails\', \'Delete\');"></span>';
				echo '<span class="adv-edit" title="Редактировать рассылку" onClick="LoadInfo('.$row["id"].', \'sent_emails\', \'LoadForm\');"></span>';
				echo '<span class="adv-erase" title="Сброс статистики" onClick="ClearStat('.$row["id"].', \'sent_emails\', \'ClearStat\', '.$row["sent"].', '.$row["nosent"].');"></span>';
				echo '<span class="adv-info" title="Предварительный просмотр рассылки" onClick="LoadInfo('.$row["id"].', \'sent_emails\', \'GetInfo\');"></span>';
			echo '</td>';
