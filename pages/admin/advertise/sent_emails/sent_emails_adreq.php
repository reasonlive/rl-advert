...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Неоплаченные заказы рассылок на e-mail</b></h3>';

require("navigator/navigator.php");
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_emails` WHERE `status`='0'");
		...
		$.ajax({
			...
			success: function(data) { 
				...
				if (result == "OK") {
					...
					$("#adv_dell"+id).remove(); $("#load-info"+id).remove(); $("#hide"+id).remove();
					AddRow("newform");
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="center" width="35">';
				echo '<div id="playpauseimg'.$row["id"].'">';
					echo '<span class="adv-play" title="Запустить рассылку на e-mail" onClick="AdsReq('.$row["id"].', \'sent_emails\', \'Start\');"></span>';
				echo '</div>';
			echo '</td>';

			echo '<td align="left">';
				echo '<span class="title-text">'.$row["subject"].'</span><br>';
				...
				echo '<span class="adv-dell" title="Удалить заказ рассылки" onClick="AdsReq('.$row["id"].', \'sent_emails\', \'Delete\');"></span>';
