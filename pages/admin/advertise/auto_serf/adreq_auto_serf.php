...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Неоплаченные заказы авто-серфинга</b></h3>';
...
if(isset($_POST["id"])) {
	...
	if($option=="add")	{
		$mysqli->query("UPDATE `tb_ads_auto` SET `status`='1', `date`='".time()."' WHERE `id`='$id'") or die($mysqli->error);
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_auto` WHERE `status`='0' ORDER BY `id` ASC");
if($sql->num_rows>0) {
