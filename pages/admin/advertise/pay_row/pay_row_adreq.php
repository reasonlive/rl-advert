...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Неоплаченные заказы рекламы в платной строке</b></h3>';
...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_pay_row` WHERE `status`='0'");
...
function AdsReq(id, type, op) {
	if (op == "Delete" && !confirm("Вы уверены что хотите удалить рекламную площадку ID: "+id+" ?")) {
		return false;
	} else if (op == "Start" && !confirm("Запустить рекламу с ID: "+id+" ?")) {
		return false;
	} else {
		$.ajax({
			type: "POST", url: "ajax_admin/ajax_admin_advertise.php?rnd="+Math.random(), 
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_pay_row` WHERE `status`='0' ORDER BY `id` DESC LIMIT $start_pos, $perpage");
if($sql->num_rows>0) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="center" width="35">';
				echo '<div id="playpauseimg'.$row["id"].'">';
					echo '<span class="adv-play" title="Запустить рекламу" onClick="AdsReq('.$row["id"].', \'pay_row\', \'Start\');"></span>';
				echo '</div>';
			echo '</td>';
			...
			echo '<td align="left">';
				...
				echo '<span class="adv-dell" title="Удалить заказ" onClick="AdsReq('.$row["id"].', \'pay_row\', \'Delete\');"></span>';
			echo '</td>';
