...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Добавление ссылки в рекламную цепочку</b></h3>';
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	$sql_id = $mysqli->query("SELECT * FROM `tb_ads_rc` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
	if($sql_id->num_rows>0) {
		$row = $sql_id->fetch_array();

		$mysqli->query("UPDATE `tb_ads_rc` SET `status`='1', `date`='".time()."' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_rc` SET `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `url`='$url', `description`='$description', `color`='$color', `ip`='$laip', `money`='0' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_rc` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`date`,`url`,`description`,`color`,`ip`,`money`) 
			VALUES('0','".session_id()."','0','$method_pay','$wmid_user','$username','".time()."','$url','$description','$color','$laip','0')") or die($mysqli->error);
		}
