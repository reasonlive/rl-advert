...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Статьи в каталог требующие модерации</b></h3>';

require("navigator/navigator.php");
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_articles` WHERE `status`='2'");
...
function AddRow(id){
	...
	if(table.rows.length < 1) {
		...
		newcell.innerHTML = "<b>Статей требущих модерации нет!</b>";
	}
}
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_articles` WHERE `status`='2' ORDER BY `id` DESC LIMIT $start_pos, $perpage");
if($sql->num_rows>0) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="center" width="35">';
				echo '<div id="playpauseimg'.$row["id"].'">';
					echo '<span class="adv-play" title="Опубликовать статью" onClick="AdsReq('.$row["id"].', \'articles\', \'Start\');"></span>';
				echo '</div>';
			echo '</td>';
	...
	echo '<tr align="center">';
		echo '<td colspan="3"><b>Статей требущих модерации нет!</b></td>';
	echo '</tr>';
}
echo '</tbody>';
echo '</table>';
