...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Заблокированные статьи в каталоге</b></h3>';

require("navigator/navigator.php");
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_articles` WHERE `status`='4'");
...
function AdsReq(id, type, op) {
	if (op == "Delete" && !confirm("Вы уверены что хотите удалить рекламную площадку ID: "+id+" ?")) {
		return false;
	} else if (op == "Start" && !confirm("Разблокировать и опубликовать статью с ID: "+id+" ?")) {
		return false;
	} else {
		$.ajax({
			type: "POST", url: "ajax_admin/ajax_admin_advertise.php?rnd="+Math.random(), 
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_articles` WHERE `status`='4' ORDER BY `id` DESC LIMIT $start_pos, $perpage");
if($sql->num_rows>0) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="center" width="35">';
				echo '<div id="playpauseimg'.$row["id"].'">';
					echo '<span class="adv-block" title="Разблокировать и опубликовать статью" onClick="AdsReq('.$row["id"].', \'articles\', \'Start\');"></span>';
				echo '</div>';
			echo '</td>';
