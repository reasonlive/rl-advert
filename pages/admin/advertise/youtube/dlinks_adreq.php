...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Неоплаченные заказы YouTube серфинга</b></h1>';
...
if(count($_GET)>0) {
	...
	if($option=="add") {
		$sql = $mysqli->query("SELECT * FROM `tb_ads_youtube` WHERE `id`='$id' AND `status`='0'");
		if($sql->num_rows>0) {
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_youtube` WHERE `status`='0' AND `type_serf`!='10' ORDER BY `id` ASC");
if($sql->num_rows>0) {
