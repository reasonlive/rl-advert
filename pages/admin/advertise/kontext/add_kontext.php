...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Добавление контекстной рекламы</b></h3>';
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_kontext` SET `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `plan`='$plan', `totals`='$plan', `url`='$url', `title`='$title', `description`='$description', `color`='$color', `ip`='$laip', `money`='0' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			...
			VALUES('0','".session_id()."','0','$method_pay','$wmid_user','$username','".time()."','$plan','$plan','$url','$title','$description','$color','$laip','0')") or die($mysqli->error);
		}
		...
		echo '<table class="tables" style="width:600px">';
			...
			echo '<tr><td><b>Количество переходов:</b></td><td>'.$plan.'</td></tr>';
...
function SbmFormB() {
	...
	for (var i=0;i<arrayElem.length;i++){
		...
		if ((document.forms["formzakaz"].title.value == '')|(document.forms["formzakaz"].title.value == 'http://')) {
