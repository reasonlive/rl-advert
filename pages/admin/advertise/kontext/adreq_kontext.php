...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Неоплаченные заказы контекстной рекламы</b></h3>';
...
if(isset($_POST["id"])) {
	...
	if($option=="add")	{
		$mysqli->query("UPDATE `tb_ads_kontext` SET `status`='1' WHERE `id`='$id'") or die($mysqli->error);
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_kontext` WHERE `status`='0' ORDER BY `id` ASC");
if($sql->num_rows>0) {
	while ($row = $sql->fetch_array()) {
		...
		if(strlen($row["title"])>40) {
			echo '<td>'.limitatexto($row["title"],40).' ....</td>';
		}else{
