...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Просмотр и редактирование ссылок в авто-серфинге</b> <b><span style="color: #3F3F3F;">You</span><span style="border-radius: 5px; background-color: #E62117; padding: 2px; color: #FFFFFF;">Tube</span></b></h3>';
...
$mysqli->query("UPDATE `tb_ads_autoyoutube` SET `status`='3', `date`='".time()."' WHERE `status`>'0' AND `status`<'3' AND `totals`<'1' ") or die($mysqli->error);
...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_autoyoutube` WHERE `status`>'0' $WHERE_ADD");
...
if(isset($_GET["option"])) {
	...
	if($country_to!=false) {$country_to="$country_to";}else{$country_to="НЕТ";}
	    ...
	    $ytflag=1;
				...
				$sql_totals = $mysqli->query("SELECT `plan`,`totals` FROM `tb_ads_autoyoutube` WHERE `id`='$id'");
				if($sql_totals->num_rows>0) {
					...
					$mysqli->query("UPDATE `tb_ads_autoyoutube` SET `img_youtube`='$img_youtube',`wmid`='$wmid',`username`='$username',`timer`='$timer',`plan`='$new_plan',`totals`='$new_totals',`limits`='$limits_table',`limits_now`='$limits_table',`url`='$url',`description`='$description' WHERE `id`='$id'") or die($mysqli->error);
				}
		...
		if(!isset($save)) {
			...
			$sql = $mysqli->query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id' ORDER BY `id` ASC");
			if($sql->num_rows>0) {
				$row = $sql->fetch_array();
	...
	if($option=="dell") {
		$sql = $mysqli->query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id' AND `status`>'0'");
		if($sql->num_rows>0) {
...
$sql_ads = $mysqli->query("SELECT * FROM `tb_ads_autoyoutube` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die($mysqli->error);
...
echo '<td align="center" valign="middle" style="border-left:solid 2px #FFFFFF;">';
	...
	echo '<table class="adv-cab" style="width:autoyoutube; margin:0; padding:0;">';
...
echo '<table class="tables" style="margin:1px autoyoutube;">';
	...
	while ($row = $sql_ads->fetch_assoc()) {
		...
		echo '<td align="center" width="30" class="noborder1">';
			echo '<div id="playpauseimg'.$row["id"].'">';
				if($row["status"]=="1") {
					echo '<span class="adv-pause" title="Приостановить показ рекламной площадки" onClick="play_pause('.$row["id"].', \'autoyoutubeserf\');"></span>';
				}elseif($row["status"]=="2") {
					echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="play_pause('.$row["id"].', \'autoyoutubeserf\');"></span>';
				}elseif($row["status"]=="3") {
