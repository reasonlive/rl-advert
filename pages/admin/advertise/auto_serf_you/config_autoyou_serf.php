...
echo '<h3 class="sp" style="margin-top:0; padding-top:0; width:600px"><b>Настройки Авто-серфинга</b> <b><span style="color: #3F3F3F;">You</span><span style="border-radius: 5px; background-color: #E62117; padding: 2px; color: #FFFFFF;">Tube</span></b></h3>';
...
if(count($_POST)>0) {
	...
	$mysqli->query("UPDATE `tb_config` SET `price`='$cena_hits_aserf' WHERE `item`='youtube_cena_hits_aserf' AND `howmany`='1'") or die($mysqli->error);
	$mysqli->query("UPDATE `tb_config` SET `price`='$nacenka_hits_aserf' WHERE `item`='youtube_nacenka_hits_aserf' AND `howmany`='1'") or die($mysqli->error);
	$mysqli->query("UPDATE `tb_config` SET `price`='$min_hits_aserf' WHERE `item`='youtube_min_hits_aserf' AND `howmany`='1'") or die($mysqli->error);
	$mysqli->query("UPDATE `tb_config` SET `price`='$timer_aserf_ot' WHERE `item`='youtube_timer_aserf_ot' AND `howmany`='1'") or die($mysqli->error);
	$mysqli->query("UPDATE `tb_config` SET `price`='$timer_aserf_do' WHERE `item`='youtube_timer_aserf_do' AND `howmany`='1'") or die($mysqli->error);
	$mysqli->query("UPDATE `tb_config` SET `price`='$cena_timer_aserf' WHERE `item`='youtube_cena_timer_aserf' AND `howmany`='1'") or die($mysqli->error);
