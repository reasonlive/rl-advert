...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Просмотр жалоб на рекламу в письмах</b></h1>';

$mysqli->query("UPDATE `tb_ads_mails` SET `status`='3' WHERE `status`>'0' AND ( (`totals`<'1' AND `nolimit`='0') OR ( `nolimit`>'0' AND `nolimit`<='".time()."') )") or die($mysqli->error);
	...
	$.ajax({
		...
		success: function(data) { 
			...
			if (data.result == "OK") { 
				...
				if(lock) {
					$("#lock-"+id).attr({class: "adv-lock", title: "Заблокировать рекламную площадку"});
					$("#load-info"+id).hide();
					$("#mess-info"+id).html("");
		...
		$.ajax({
			...
			success: function(data) { 
				...
				if (data.result == "OK") {
					$("#load-info"+id).hide();
					$("#mess-info"+id).html("");
					$("#playpauseimg"+id).html('<span class="adv-block" title="Рекламная площадка заблокирована" onClick="PlayPause('+id+', \'mails\', \'PlayPause\', \'1\');"></span>');
...
$perpage = 20;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_mails` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='mails') ");
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_mails` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='mails') ORDER BY `id` DESC LIMIT $start_pos,$perpage ");
if($sql->num_rows>0) {
	while ($row = $sql->fetch_assoc()) {
		...
		echo '<td align="center" width="30" class="noborder1" style="border-right:solid 1px #DDDDDD;">';
			echo '<div id="playpauseimg'.$row["id"].'">';
				if($row["status"]=="0") {
					...
					echo '<span class="adv-pause" title="Приостановить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'mails\', \'PlayPause\');"></span>';
				}elseif($row["status"]=="2") {
					echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'mails\', \'PlayPause\');"></span>';
				}elseif($row["status"]=="3") {
					echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="PlayPause('.$row["id"].', \'mails\', \'PlayPause\');"></span>';
				}elseif($row["status"]=="4") {
					echo '<span class="adv-block" title="Рекламная площадка заблокирована" onClick="PlayPause('.$row["id"].', \'mails\', \'PlayPause\', \'1\');"></span>';
				}
			echo '</div>';
		echo '</td>';
		...
		echo '<td width="100">';
			echo '<span class="adv-dell" title="Удалить рекламную площадку" onClick="DelAdv('.$row["id"].', \'mails\', \'Delete\');"></span>';
			echo '<span id="lock-'.$row["id"].'" class="adv-'.($row["status"]=="4" ? "unlock" : "lock").'" title="'.($row["status"]=="4" ? "Просмотр информации о блокировки" : "Заблокировать тест").'" onClick="LoadInfo('.$row["id"].', \'mails\', \'GoLock\');"></span>';
			echo '<span class="clear-claims" title="Удалить все жалобы" onClick="DelAllClaims(\''.$row["id"].'\', \'mails\', \'DelAllClaims\');"></span>';
			echo '<span class="view-claims" title="Просмотр жалоб" onClick="ViewClaims(\''.$row["id"].'\', \'mails\', \'ViewClaims\');"></span>';
		echo '</td>';
