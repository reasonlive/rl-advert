...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Редактирование статической ссылки</b></h3>';
...
$perpage = 25;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_slink` WHERE `status`>'0' $WHERE_ADD");
...
if(isset($_GET["option"])) {
	...
	if($option=="edit") {
		...
		if(count($_POST)>0) {
			...
			if($sql_bl->num_rows>0 && $black_url!=false) {
				...
				$mysqli->query("UPDATE `tb_ads_slink` SET `wmid`='$wmid', `username`='$username', `date_end`=`date`+'".($plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `color`='$color' WHERE `id`='$id'") or die($mysqli->error);
		...
		if(!isset($save)) {
			...
			$sql = $mysqli->query("SELECT * FROM `tb_ads_slink` WHERE `id`='$id' ORDER BY `id` ASC");
			if($sql->num_rows>0) {
				$row = $sql->fetch_array();
	...
	if($option=="dell") {
		$mysqli->query("DELETE FROM `tb_ads_slink` WHERE `id`='$id'") or die($mysqli->error);
...
$sql_ads = $mysqli->query("SELECT * FROM `tb_ads_slink` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die($mysqli->error);
