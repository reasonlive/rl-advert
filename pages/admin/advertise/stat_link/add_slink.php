...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	if($sql_id->num_rows>0) {
		...
		$mysqli->query("UPDATE `tb_ads_slink` SET `status`='1', `date`='".time()."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_slink` SET `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `color`='$color', `ip`='$laip', `money`='0' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
