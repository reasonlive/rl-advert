...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;"><b>Редактирование псевдо-динамической рекламы</b></h3>';
...
$perpage = 25;
$sql_p = $mysqli->query("SELECT `id` FROM `tb_ads_psevdo` WHERE `status`>'0' $WHERE_ADD");
...
if(isset($_GET["option"])) {
	...
	if($option=="edit") {
		...
		if(count($_POST)>0) {
			...
			if($sql_bl->num_rows>0 && $black_url!=false) {
				...
				$save = "ok";
				$mysqli->query("UPDATE `tb_ads_psevdo` SET `wmid`='$wmid', `username`='$username', `date_end`=`date`+'".($plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `color`='$color' WHERE `id`='$id'") or die($mysqli->error);
		...
		if(!isset($save)) {
			...
			$sql = $mysqli->query("SELECT * FROM `tb_ads_psevdo` WHERE `id`='$id' AND `status`='1' ORDER BY `id` ASC");
			if($sql->num_rows>0) {
				$row = $sql->fetch_array();
	...
	if($option=="dell") {
		$mysqli->query("DELETE FROM `tb_ads_psevdo` WHERE `id`='$id'") or die($mysqli->error);
...
$sql_ads = $mysqli->query("SELECT * FROM `tb_ads_psevdo` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die($mysqli->error);
