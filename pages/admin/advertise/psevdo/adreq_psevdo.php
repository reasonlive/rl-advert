...
if(isset($_POST["id"])) {
	...
	if($option=="add")	{
		$mysqli->query("UPDATE `tb_ads_psevdo` SET `status`='1' WHERE `id`='$id'") or die($mysqli->error);
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_psevdo` WHERE `status`='0' ORDER BY `id` ASC");
if($sql->num_rows>0) {
