...
echo '<h3 class="sp" style="margin-top:0; padding-top:0;">Просмотр и редактирование <b>"Быстрые сообщения"</b></h3>';

$system_pay[0] = "Админка";
$system_pay[1] = "Осн. счет";
$system_pay[2] = "Рекл. счет";
?>
...
$sql = $mysqli->query("SELECT * FROM `tb_ads_quick_mess` WHERE `status`='1' ORDER BY `id` DESC LIMIT 10");
if($sql->num_rows>0) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			echo '<td align="left" style="border-right:solid 1px #DDDDDD;">';
				echo '<div id="adv-'.$row["id"].'" style="margin-bottom:4px;">';
					...
					echo 'URL: '.($row["url"]!=false ? '<a href="'.$row["url"].'" target="_blank">'.$row["url"].'</a>' : '<span style="color:#CCC;">нет</span>').'';
				echo '</div>';

				echo '<span class="info-text">';
					echo 'ID:&nbsp;<b>'.$row["id"].'</b>;&nbsp;&nbsp;Способ оплаты: <b>'.$system_pay[$row["method_pay"]].'</b>;&nbsp;&nbsp;';
					echo 'Рекламодатель: '.($row["username"]!=false ? '<b>'.$row["username"].'</b>' : '<span style="color:#CCC;">не опеределен</span>').'';
				echo '</span>';
		
				echo '<span class="adv-dell" title="Удалить рекламную площадку" onClick="AdsReq('.$row["id"].', \'quick_mess\', \'Delete\');"></span>';
				echo '<span class="adv-edit" title="Редактировать рекламную площадку" onClick="LoadInfo('.$row["id"].', \'quick_mess\', \'LoadForm\');"></span>';
			echo '</td>';
	...
	echo '<tr align="center"><td colspan="2"><b>Реклама не найдена!</b></td></tr>';
}
echo '</tbody>';
echo '</table>';
