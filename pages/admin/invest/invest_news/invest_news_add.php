...
function AddNews() {
	...
	if (title == "") {
		...
		$.ajax({
			...
			data: {'op':'AddNews', 'title':title, 'description':description}, 
			...
			success: function(data) { 
				...
				if (data == "OK") {
					$("#info-msg-news").html('<span class="msg-ok">Новость успешно добавлена.</span>').slideToggle("fast");
					ClearForm(); HideMsg("info-msg-news", 2000);
					return false;
				} else {
...
echo '<tbody>';
	echo '<tr>';
		...
		echo '<td align="left"><input type="text" id="title" value="" maxlength="60" class="ok" autocomplete="off" onKeyDown="$(this).attr(\'class\', \'ok\');" /></td>';
	echo '</tr>';
	echo '<tr>';
	...
	echo '<tr>';
		echo '<td colspan="2">';
			...
			echo '<div style="display: block; clear:both; padding-top:4px">';
				echo '<textarea id="description" class="ok" style="height:300px; width:99.2%;" onKeyDown="$(this).attr(\'class\', \'ok\');"></textarea>';
			echo '</div>';
		echo '</td>';
	echo '</tr>';
echo '</tbody>';
