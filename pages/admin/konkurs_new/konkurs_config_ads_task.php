...
if( isset($_POST["type"]) && limpiar($_POST["type"])=="ads_task" ) {
	...
	for($y=1; $y<=3; $y++) {
		...
		for($i=1; $i<=3; $i++) {
			...
			if(($y==2 | $y==3) && $konk_ads_task_type_prize[$y]==1) $konk_ads_task_prizes[$y][$i] = intval($konk_ads_task_prizes[$y][$i]);
	...
	for($i=1; $i<=3; $i++) {
		if(($konk_ads_task_prizes[1][$i] + $konk_ads_task_prizes[2][$i] + $konk_ads_task_prizes[3][$i])!=0) $konk_ads_task_all_count_prize++;
	}
	...
	for($y=1; $y<=3; $y++) {
		for($i=1; $i<=3; $i++) {
			...
			if($konk_ads_task_type_prize_error==5) {
				...
				break;
			}elseif($i<=1 && $konk_ads_task_prizes[1][$i]==0 && $konk_ads_task_prizes[2][$i]==0 && $konk_ads_task_prizes[3][$i]==0) {
				for($z=1; $z<=3; $z++) {
					if($konk_ads_task_type_prize[$z]==1 && $konk_ads_task_prizes[$z][$i]==0) {
						$error++;
						echo '<span class="msg-error">Необходимо указать приз<br>'.str_ireplace("<br>"," ", $konk_ads_task_type_prize_arr[$z]).' за '.$i.' место!</span>';
						break;
					}
				}
			}elseif($i>1 && $i<3 && ($konk_ads_task_prizes[1][$i]==0 && $konk_ads_task_prizes[2][$i]==0 && $konk_ads_task_prizes[3][$i]==0 && $konk_ads_task_prizes[4][$i]==0) && ($konk_ads_task_prizes[1][$i+1]>0 | $konk_ads_task_prizes[2][$i+1]>0 | $konk_ads_task_prizes[3][$i+1]>0)) {
				$error++;
				...
				$error++;
				echo '<span class="msg-error">Приз: '.str_ireplace("<br>"," ", $konk_ads_task_type_prize_arr[$y]).' - за '.$i.' место не может быть больше чем за '.($i-1).' место</span>';
				break;
			}
		}
	}
	...
	if($error==0 && $konk_ads_task_date_end==$konk_ads_task_date_start) {
		...
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_title' WHERE `type`='ads_task' AND `item`='title'");

		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='$konk_ads_task_date_start' WHERE `type`='ads_task' AND `item`='date_start'");
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='$konk_ads_task_date_end' WHERE `type`='ads_task' AND `item`='date_end'");
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='$konk_ads_task_min' WHERE `type`='ads_task' AND `item`='min_do'");
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='$konk_ads_task_autostart' WHERE `type`='ads_task' AND `item`='autostart'");
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='$konk_ads_task_all_count_prize' WHERE `type`='ads_task' AND `item`='all_count_prize'");

		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_count_prize' WHERE `type`='ads_task' AND `item`='count_prize'");

		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_type_prize' WHERE `type`='ads_task' AND `item`='type_prize'");

		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_prizes_1' WHERE `type`='ads_task' AND `item`='prizes' AND `howmany`='1'");
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_prizes_2' WHERE `type`='ads_task' AND `item`='prizes' AND `howmany`='2'");
		$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_prizes_3' WHERE `type`='ads_task' AND `item`='prizes' AND `howmany`='3'");
		//$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_prizes_4' WHERE `type`='ads_task' AND `item`='prizes' AND `howmany`='4'");
		//$mysqli->query("UPDATE `tb_konkurs_conf` SET `price_array`='$konk_ads_task_prizes_5' WHERE `type`='ads_task' AND `item`='prizes' AND `howmany`='5'");

		if($konk_ads_task_date_end>=time()) {
			$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='1' WHERE `type`='ads_task' AND `item`='status'");
		}else{
			$mysqli->query("UPDATE `tb_konkurs_conf` SET `price`='0' WHERE `type`='ads_task' AND `item`='status'");
		}
...
$sql = $mysqli->query("SELECT `price_array` FROM `tb_konkurs_conf` WHERE `type`='ads_task' AND `item`='title'");
$konk_ads_task_title = $sql->fetch_object()->price_array;
...
if(count($_POST)>0) {
	...
	$sql = $mysqli->query("SELECT `price_array` FROM `tb_konkurs_conf` WHERE `type`='ads_task' AND `item`='count_prize'");
	$konk_ads_task_count_prize = explode("; ", $sql->fetch_object()->price_array);

	$sql = $mysqli->query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='ads_task' AND `item`='min_do'");
	$konk_ads_task_min = $sql->fetch_object()->price;

	$sql = $mysqli->query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='ads_task' AND `item`='autostart'");
	$konk_ads_task_autostart = $sql->fetch_object()->price;

	$sql = $mysqli->query("SELECT `price_array` FROM `tb_konkurs_conf` WHERE `type`='ads_task' AND `item`='type_prize'");
	$konk_ads_task_type_prize = explode("; ", $sql->fetch_object()->price_array);

	for($y=1; $y<=3; $y++) {
		$sql = $mysqli->query("SELECT `price_array` FROM `tb_konkurs_conf` WHERE `type`='ads_task' AND `item`='prizes' AND `howmany`='$y'");
		$konk_ads_task_prizes[$y] = explode("; ", $sql->fetch_object()->price_array);
	}
}
...
echo '<tbody>';
	echo '<tr>';
		...
		echo '<td align="left">'.($konk_ads_task_status==1 ? '<b style="color:green;">Активен</b>' : '<b style="color:red;">Не активен</b>').'</td>';
	echo '</tr>';
	echo '<tr>';
