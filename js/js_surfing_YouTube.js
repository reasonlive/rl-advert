...
function StartSurfing(id, url) {
	$("#"+id).css("cursor","default").html('<div style="text-align:center; color:#2E8B57; font-size:14px; display:block; margin:0 auto; padding:0;">Спасибо за просмотр!</div>');
...
function fixed(id) {
	$.ajax({
		type: "POST", url: "/ajax/ajax_youtube.php?rnd="+Math.random(), 
		data: {'id':id}, 
		...
		success: function(data) {
			...
			if(obj_data.result=="OK") {
				$("#"+id).attr({class:"td-serf"}).html('<div id="link_'+id+'" align="center" onClick="StartSurfing(\''+id+'\', \''+obj_data.url_link+'\');"><span class="startserf">Перейти к просмотру видеоролика рекламодателя</span></div>');
				if($("#info_serf_"+id).css("display")!="none") $("#info_serf_"+id).css("display", "none");
				if($("#claims_"+id).css("display")!="none") $("#claims_"+id).css("display", "none");
