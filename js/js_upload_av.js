	...
	$.ajax_upload (button, {
		...
		onSubmit: function(file, ext){
			if (! (ext && /^(jpg|png|jpeg|gif)$/.test(ext))){
				$("#info-msg").html('<span class="msg-error">Допустимые форматы аватара для загрузки - gif, png, jpeg, jpg</span>').show();
				setTimeout(function() {$("#info-msg").fadeOut('slow')}, 3000);
				return false;
			}
			$("#info-msg").html('<span id="loading" title="Подождите пожалуйста..."></span>').show();
			this.disable();
		},
		...
		onComplete: function(file, response){
			...
			if (response == "SUCCES") {
				...
				$("#info-msg").html('<span class="msg-error">Ошибка!</span>').show();
				setTimeout(function() {$("#info-msg").fadeOut('slow')}, 3000);
			} else {
				$("#info-msg").html('<span class="msg-error">'+response+'</span>').show();
				setTimeout(function() {$("#info-msg").fadeOut('slow')}, 3000);
			}
		}
	});
