...
;(function($) {
  ...
  $.fn.eTimer = function(options) {
	...
	var now = new Date();
        if (config.etType == 1) {
          date = Date.UTC(now.getFullYear(), now.getMonth(), now.getDate() + 1);
          ...
          date = date.split('.');
          date = Date.UTC(date[2], date[1] - 1, date[0], date[3], date[4]);
      ...
      element.tick = function() {
        var timeLeft = Math.floor((date - Date.parse(new Date()) - 3*60*60*1000) / 1000),
          unit;
