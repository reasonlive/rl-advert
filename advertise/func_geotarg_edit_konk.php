...
for ($i = 1; $i <= count($geo_codname_arr); $i++) {
	if($i==1 | $i==5 | $i==9 | $i==13 | $i==17 | $i==21 | $i==25 | $i==29 | $i==33) echo '<tr>';
		echo '<td width="25%">';
			echo '<input type="checkbox" id="country[]" name="country[]" value="'.$geo_codname_arr[$i]["0"].'" '.(strpos(strtoupper($konk_hit_ed_country), strtoupper($geo_codname_arr[$i]["0"])) !== false ? 'checked="checked"' : false).' />';
