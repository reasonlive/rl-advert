...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = $mysqli->query("SELECT * FROM `tb_ads_auto` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if($sql_id->num_rows>0) {
			...
			if($money_user>=$money_pay) {
				...
				$mysqli->query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: автосерфинг - $plan шт.','Списано','rashod')") or die($mysqli->error);
...
if(count($_POST)>0) {
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		echo '<script type="text/javascript"> top.document.location.href = "http://'.$_SERVER["HTTP_HOST"].'/check_url_as.php"; </script>';
...
$system_pay[21] = "AdvCash";
		...
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_auto` SET `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`wmid`='$wmid_user',`username`='$username', `geo_targ`='$country', `timer`='$timer', `date`='".time()."', `plan`='$plan', `totals`='$plan', `limits`='$limits_table',`limits_now`='$limits_table', `url`='$url', `description`='$description', `check_url`='1', `ip`='$laip', `money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_auto` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`geo_targ`,`timer`,`date`,`plan`,`totals`,`limits`,`limits_now`,`members`,`url`,`description`,`check_url`,`claims`,`ip`,`money`) 
			VALUES('0','".session_id()."','$merch_tran_id','$method_pay','$wmid_user','$username','$country','$timer','".time()."','$plan','$plan','$limits_table','$limits_table','0','$url','$description','1','0','$laip','$summa')") or die($mysqli->error);
		}
		...
		$shp_item = "5";
		$inv_desc = "Оплата рекламы: авто-серфинг, план:$plan, счет:$merch_tran_id";
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Авто-серфинг - что это?</span>';
	...
	echo '<table class="tables">';
		...
		if(isset($_SESSION["checkurl"]) | isset($_SESSION["checkurl_ok"])) {
			...
			echo '<tr>';
				...
				echo '<td><input type="text" name="plan" id="plan" maxlength="7" value="100" class="ok12" style="text-align:center;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;&nbsp;(минимум - '.$min_hits_aserf.')</td>';
			echo '</tr>';
			echo '<tr>';
