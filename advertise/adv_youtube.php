...
$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_hits' AND `howmany`='1'");
$youtube_cena_hits = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_hits_bs' AND `howmany`='1'");
$youtube_cena_hits_bs = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_nacenka' AND `howmany`='1'");
$youtube_nacenka = $sql->fetch_object()->price;
...
$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_min_hits_vip' AND `howmany`='1'");
$youtube_min_hits_vip = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_color' AND `howmany`='1'");
$youtube_cena_color = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_active' AND `howmany`='1'");
$youtube_cena_active = $sql->fetch_object()->price;
...
$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_timer' AND `howmany`='1'");
$youtube_cena_timer = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_revisit' AND `howmany`='1'");
$youtube_cena_revisit_1 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_revisit' AND `howmany`='2'");
$youtube_cena_revisit_2 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_nolimit' AND `howmany`='0'");
$youtube_cena_nolimit = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_nolimit' AND `howmany`='1'");
$youtube_cena_nolimit_1 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_nolimit' AND `howmany`='2'");
$youtube_cena_nolimit_2 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_nolimit' AND `howmany`='3'");
$youtube_cena_nolimit_3 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_nolimit' AND `howmany`='4'");
$youtube_cena_nolimit_4 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_unic_ip' AND `howmany`='1'");
$youtube_cena_unic_ip_1 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_unic_ip' AND `howmany`='2'");
$youtube_cena_unic_ip_2 = $sql->fetch_object()->price;

$sql = $mysqli->query("SELECT `price` FROM `tb_config` WHERE `item`='youtube_cena_uplist' AND `howmany`='1'");
$youtube_cena_uplist = $sql->fetch_object()->price;
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = $mysqli->query("SELECT * FROM `tb_ads_youtube` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if($sql_id->num_rows>0) {
			...
			if($money_user>=$money_pay) {
			   ...
			   $mysqli->query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1'+'$reit_add_3', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay' $vip_serf_tab WHERE `username`='$username'") or die($mysqli->error);
				...
				VALUES('$username', '".DATE("d.m.Y H:i")."', '$money_pay', 'Оплата рекламы [YouTube], ID:#$id_pay', 'Оплачено', 'reklama')") or die($mysqli->error);
...
if(count($_POST)>0) {
	...
	/*}elseif(($type_serf==2 | $type_serf==4) && ($description==false | $description=="http://" | $description=="https://")) {
	...
	}elseif($nolimit==0 && ($type_serf==1 | $type_serf==2) && $plan<$youtube_min_hits) {
		echo '<span class="msg-error">Минимальный заказ - '.$youtube_min_hits.' показов.</span><br>';
	}elseif($nolimit==0 && ($type_serf==3 | $type_serf==4) && $plan<$youtube_min_hits_vip) {
		echo '<span class="msg-error">Минимальный заказ - '.$youtube_min_hits_vip.' показов.</span><br>';
		...
		echo '<span class="msg-error">Для VIP серфинга время просмотра должно быть в пределах от 20 сек. до '.$youtube_timer_do.' сек.</span>';
...
$system_pay[21] = "AdvCash";
		...
		$color_to[1]="ДА (".number_format(($plan * $color * $youtube_cena_color),4,".","")." руб.)";

		$up_list_to[0]="НЕТ";
		$up_list_to[1]="ДА (".number_format(($up_list * $youtube_cena_uplist),4,".","")." руб.)";
		...
		$unic_ip_to[0]="НЕТ";
		$unic_ip_to[1]="Да (100% совпадение) &mdash; (".number_format(($plan * $youtube_cena_unic_ip_1),4,".","")." руб.)";
		$unic_ip_to[2]="Усиленный по маске до 2 чисел (255.255.X.X) &mdash; (".number_format(($plan * $youtube_cena_unic_ip_2),4,".","")." руб.)";
		...
		$active_to[1]="ДА (".number_format(($plan * $active * $youtube_cena_active * (100+$youtube_nacenka)/100),4,".","")." руб./просмотр)";

		if($timer>$youtube_timer_ot) {
			$timer_to = "$timer (".number_format(($plan * ($timer-$youtube_timer_ot) * $youtube_cena_timer * (100+$youtube_nacenka)/100),4,".","")." руб.)";
		}else{
		...
		if($revisit==0) {$youtube_cena_revisit = 0; $revisit_to = "Каждые 24 часа (0.00 руб./просмотр)";}
		elseif($revisit==1) {$youtube_cena_revisit = $youtube_cena_revisit_1; $revisit_to = "Каждые 48 часов (".number_format(($youtube_cena_revisit_1*$plan),4,".","")." руб./просмотр)";}
		elseif($revisit==2) {$youtube_cena_revisit = $youtube_cena_revisit_2; $revisit_to = "1 раз (".number_format(($youtube_cena_revisit_2*$plan),4,".","")." руб./просмотр)";}

		if($unic_ip==0) { $youtube_cena_inic_ip = 0; }
		else if($unic_ip==1) { $youtube_cena_inic_ip = $youtube_cena_unic_ip_1; }
		else if($unic_ip==2) { $youtube_cena_inic_ip = $youtube_cena_unic_ip_2; }

		if($nolimit==1) {
			$summa = number_format($youtube_cena_nolimit_1,2,".","");
		}elseif($nolimit==2) {
			$summa = number_format($youtube_cena_nolimit_2,2,".","");
		}elseif($nolimit==3) {
			$summa = number_format($youtube_cena_nolimit_3,2,".","");
		}elseif($nolimit==4) {
			$summa = number_format($youtube_cena_nolimit_4,2,".","");
		}else{
			if($type_serf==2 | $type_serf==4) $youtube_cena_hits = $youtube_cena_hits_bs;

			$precio = $plan * ($youtube_cena_hits + ($youtube_cena_timer * ($timer-$youtube_timer_ot)) + ($active * $youtube_cena_active)) * ($youtube_nacenka + 100)/100 + $plan * ( ($color * $youtube_cena_color) + $youtube_cena_revisit + $youtube_cena_inic_ip) + ($up_list * $youtube_cena_uplist);
		...
		if($sql_check->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_youtube` SET `img_youtube`='$img_youtube', `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`type_serf`='$type_serf',`date`='".time()."',`up_list`='".($up_list * time())."',`wmid`='$wmid_user',`username`='$username',`geo_targ`='$country',`content`='$content',`active`='$active',`revisit`='$revisit',`color`='$color',`timer`='$timer',`nolimit`='$nolimitdate',`limit_d`='$limit_d',`limit_h`='$limit_h',`limit_d_now`='$limit_d',`limit_h_now`='$limit_h',`new_users`='$new_users',`unic_ip`='$unic_ip',`no_ref`='$no_ref',`sex_adv`='$sex_adv',`to_ref`='$to_ref',`url`='$url',`title`='$title',`description`='$description',`urlbanner_load`='$urlbanner_load',`plan`='$plan',`totals`='$plan',`ip`='$laip',`money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_youtube` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`type_serf`,`date`,`up_list`,`wmid`,`username`,`geo_targ`,`content`,`active`,`revisit`,`color`,`timer`,`nolimit`,`limit_d`,`limit_h`,`limit_d_now`,`limit_h_now`,`new_users`,`unic_ip`,`no_ref`,`sex_adv`,`to_ref`,`url`,`title`,`description`,`urlbanner_load`,`plan`,`totals`,`ip`,`money`, `ytflag`, `img_youtube`) 
			VALUES('0','".session_id()."','$merch_tran_id','$method_pay','$type_serf','".time()."','".($up_list * time())."','$wmid_user','$username','$country','$content','$active','$revisit','$color','$timer','$nolimitdate','$limit_d','$limit_h','$limit_d','$limit_h','$new_users','$unic_ip','$no_ref','$sex_adv','$to_ref','$url','$title','$description','$urlbanner_load','$plan','$plan','$laip','$summa','$ytflag', '$img_youtube')") or die($mysqli->error);
		}
		...
		echo '<table class="tables">';
			...
			if($type_serf==3) {
				echo '<tr><td><b>VIP серфинг</b></td><td>Вам будет доступно 200 просмотров видеороликов в VIP YouTube</td></tr>';
			}
			if($type_serf==4) {
				echo '<tr><td><b>VIP серфинг</b></td><td>Вам будет доступно 400 просмотров баннеров в VIP серфинге</td></tr>';
			}
			if($type_serf==2 | $type_serf==4) {
				...
				echo '<tr><td><b>Заголовок ролика:</b></td><td><a href="'.$url.'" target="_blank">'.$title.'</a></td></tr>';
			...
			if($nolimit>0) {
				...
				echo '<tr><td><b>Доступно для просмотра:</b></td><td>Каждые 24 часа</td></tr>';
			}else{
				echo '<tr><td><b>Количество визитов:</b></td><td>'.$plan.' ('.number_format(($plan * $youtube_cena_hits * (100+$youtube_nacenka)/100),2,".","").' руб.)</td></tr>';
		...
		if($type_serf==1) {
			$inv_desc = "Оплата рекламы: YouTube серфинг, тариф:$plan, счет:$merch_tran_id";
			$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
			$inv_desc_en = "Pay advertise: YouTube surfing, plan:$plan, order:$merch_tran_id";
		}elseif($type_serf==2) {
			...
			$inv_desc = "Оплата рекламы: YouTube серфинг(vip), тариф:$plan, счет:$merch_tran_id";
			$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
			$inv_desc_en = "Pay advertise: YouTube surfing(vip), plan:$plan, order:$merch_tran_id";
		}elseif($type_serf==4) {
			...
			$inv_desc = "Оплата рекламы: YouTube серфинг, тариф:$plan, счет:$merch_tran_id";
			$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
			$inv_desc_en = "Pay advertise: YouTube link, plan:$plan, order:$merch_tran_id";
		}

		if(($type_serf==3 | $type_serf==4) && $method_pay!=10 ) {
			echo '<span class="msg-error">Оплата VIP серфинга YouTube возможна только с рекламного счета!</span>';
			include('footer.php');
			exit();
		}
...
$(document).ready(function(){
	$("#hint1").simpletip({
		...
		content: '<b>Выберите тип серфинга YouTube</b> - выберите какой серфинг YouTube вы хотите заказать обычный или VIP.<br><b style="color:red;">Внимание!<br>Если Вы рекламодатель и хотите, чтобы Ваш видеоролик получила больше просмотров, выбирайте YouTube серфинг (не VIP). Для выделения Вашей ссылке в списке, воспользуйтесь Дополнительными настройками: выделение цветом, размещение в начале списка, показ только в активном окне.<br>Если Вы зарабатываете на просмотре видеороликах и хотите получить больше доступных просмотров, выбирайте тип серфинга YouTube-VIP.</b>'
	});
	$("#hint2").simpletip({
	...
	$("#hintyt").simpletip({
		fixed: true, position: ["-622", "-45"], focus: false,
		content: '<b>Ссылка на ролик YouTube</b> - вы можете заказать показы вашего ролика на сервисе YouTube.'
	});
	$("#hint3").simpletip({
	...
	$("#hint21").simpletip({
		fixed: true, position: ["-622", "23"], focus: false,
		content: '<b>Активное окно</b> - Ваш видеоролик будет показываться только в активном окне.'
	});
	$("#hinturl").simpletip({
		fixed: true, position: ["-622", "23"], focus: false,
...
function SbmFormB() {
	...
	} else if(nolimit == '0' && (type_serf == '3' | type_serf == '4' ) && (plan == '' | plan < <?=$youtube_min_hits_vip;?>) ) {
		$("#plan").focus().attr("class", "err12");
		alert("Минимальное количество визитов - <?=$youtube_min_hits_vip;?>");
		return false;
...
function PlanChange(){
	...
	var cena_timer = <?php echo $youtube_cena_timer;?>;
	var cena_color = <?php echo $youtube_cena_color;?>;
	var cena_active = <?php echo $youtube_cena_active;?>;
	var cena_up_list = <?php echo $youtube_cena_uplist;?>;
	var cena_nac = <?php echo (100+$youtube_nacenka)/100;?>;
	...
	if(type_serf==3 | type_serf==4) {
		$("#min_serf").html('&nbsp;&nbsp;[минимум - <?php echo $youtube_min_hits_vip;?>]');
		$("#timer_serf").html('&nbsp;&nbsp;[<?php echo "от 20 сек. до $youtube_timer_do сек.";?>]');
	} else {
		$("#min_serf").html('&nbsp;&nbsp;[минимум - <?php echo $youtube_min_hits;?>]');
		$("#timer_serf").html('&nbsp;&nbsp;[<?php echo "от $youtube_timer_ot сек. до $youtube_timer_do сек.";?>]');
	}
	...
	if(revisit==1) {var cena_revisit = <?php echo $youtube_cena_revisit_1;?>;}
	if(revisit==2) {var cena_revisit = <?php echo $youtube_cena_revisit_2;?>;}

	if(unic_ip==0) {var cena_unic_ip = 0;}
	if(unic_ip==1) {var cena_unic_ip = <?php echo $youtube_cena_unic_ip_1;?>;}
	if(unic_ip==2) {var cena_unic_ip = <?php echo $youtube_cena_unic_ip_2;?>;}

	if(nolimit==1) {
		var price = <?=@$youtube_cena_nolimit_1;?>;
	}else if(nolimit==2) {
		var price = <?=@$youtube_cena_nolimit_2;?>;
	}else if(nolimit==3) {
		var price = <?=@$youtube_cena_nolimit_3;?>;
	}else if(nolimit==4) {
		var price = <?=@$youtube_cena_nolimit_4;?>;
	}else{
		nolimit = 0;
	}
	...
	if(nolimit>0) {
		...
		$("#bl4").hide(); $("#bl41").html('');
		$("#bl5").hide(); $("#bl51").html('<b>Да</b>');
		$("#bl6").hide(); $("#bl61").html('<b>Каждые 24 часа</b>');
		...
		var price = plan * cena_nac * (cena_hits + (cena_timer * (timer-<?=$youtube_timer_ot;?>)) + (active * cena_active)) + plan * ((color * cena_color) + cena_revisit + cena_unic_ip) + (up_list * cena_up_list);
	}
...
echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
	echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Реклама в серфинге YouTube - что это?</span>';
	echo '<div id="adv-block-info" style="display:block; padding:5px 7px 10px 7px; text-align:justify; background-color:#FFFFFF;">';
		echo 'Доступная, эффективная и недорогая реклама на <b style="color:#3A5FCD">'.strtoupper($_SERVER["HTTP_HOST"]).'</b> &mdash; прекрасная возможность привлечения целевой аудитории на ваш канал или видеоролик YouTube. ';
		echo 'Тысячи потенциальных потребителей смогут в полной мере ознакомиться с вашей продукцией или услугами. ';
		echo 'Серфинг YouTube гарантирует уникальные посещения рекламируемого канала или видеоролика YouTube в течение 24 часов. ';
		echo 'Кроме того, вы можете максимально точно сформировать поток именно тех посетителей, для которых ваш канал или видеоролик будет наиболее интересен.';
	echo '</div>';
	...
	echo '<span id="adv-title-bl" class="adv-title-open" onclick="ShowHideBlock(\'-bl\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Безлимитная реклама</span>';
		echo '<div id="adv-block-bl" style="display:block; padding:2px 0px 10px 0px; text-align:center; background-color:#FFFFFF;">';
                         ...
                         echo '<b>Безлимитный серфинг YouTube.</b><br> ';
                         echo 'Вы можете заказать безлимитный серфинг YouTube от 1 до 4 недель.<br> ';
                         echo 'Стоимость безлимитного серфинга <span style="color:green;">от '.number_format($youtube_cena_nolimit_1,2,".","`").' до '.number_format($youtube_cena_nolimit_4,2,".","`").' руб...</span>.<br> ';
                         echo 'Ссылка получит до<span style="color:#6c8bff;"> '.number_format($youtube_cena_nolimit,0,".","`").'</span> просмотров.</div> ';
		echo '</div>';
//echo '</div>';
...
echo '<span id="adv-title-rules" class="adv-title-close_1" onclick="ShowHideBlock(\'-rules\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Как правильно разместить видеоролик</span>';
	...
	echo '<div id="adv-block-rules" style="display:block; padding:5px 7px 10px 7px; text-align:center; background-color:#FFFFFF;">';
		echo '<span style="display:block;font:17px/1.2em arial, helvetica, verdana, sans-serif; padding:5px 7px 10px 7px; text-align:center;color:#f3f1f0; background:#ff0000;">';
		echo 'На ролике должен быть ОТКЛЮЧЕН просмотр только в безопасном режиме и<br> должен быть РАЗРЕШЕН просмотр вашего видео на других сайтах.<br>';
	echo 'Ролики которые будут нарушать <a href="/tos.php" target="_blank">правила</a> сайта будут удаляться!';
		echo '</span>';
	echo '</div>';
echo '</div>';
...
echo '<form method="post" action="" name="formzakaz" onsubmit="return SbmFormB(); return false;" id="newform">';
	...
	echo '<tr>';
		...
		echo '<td>';
			echo '<select id="type_serf" name="type_serf" onChange="PlanChange();" onClick="PlanChange();">';
				echo '<option value="1">YouTube серфинг</option>';
				//echo '<option value="2">Баннерный серфинг</option>';
				echo '<option value="3">YouTube серфинг - VIP (Вам будет доступно 200 просмотров видео в VIP серфинге YouTube)</option>';
	...
	echo '<tr>';
		...
		echo '<td>';
			echo '<select id="nolimit" name="nolimit" onChange="PlanChange();" onClick="PlanChange();">';
				...
				echo '<option value="1">1&nbsp;неделя&nbsp;('.number_format($youtube_cena_nolimit_1,2,".","`").' руб.)</option>';
				echo '<option value="2">2&nbsp;недели&nbsp;('.number_format($youtube_cena_nolimit_2,2,".","`").' руб.)</option>';
				echo '<option value="3">3&nbsp;недели&nbsp;('.number_format($youtube_cena_nolimit_3,2,".","`").' руб.)</option>';
				echo '<option value="4">1&nbsp;&nbsp;месяц&nbsp;&nbsp;('.number_format($youtube_cena_nolimit_4,2,".","`").' руб.)</option>';
			echo '</select>';
		echo '</td>';
		echo '<td align="center" width="16" style="background: #EDEDED;"><span id="hint2" class="hint-quest"></span></td>';
	echo '</tr>';
	echo '<tr>';
		echo '<td align="left"><b>URL видео</b></td>';
		echo '<td><input type="text" id="url" name="url" maxlength="300" value="https://" class="ok" onKeyDown="$(this).attr(\'class\', \'ok\');"></td>';
	...
	echo '<div id="adv-block1" style="display:none;">';
		...
		echo '<tbody>';
			echo '<tr>';
				...
				echo '<td style="height:31px;"><div id="bl7">';
					echo '<select name="up_list" id="up_list" onChange="PlanChange();" onClick="PlanChange();">';
						echo '<option value="0">Нет</option>';
						echo '<option value="1">Да (+'.number_format($youtube_cena_uplist, 4,".","").' руб.)</option>';
					echo '</select>';
			...
			echo '<tr id="type_serf_d">';
				...
				echo '<td style="height:31px;"><div id="bl3"><select name="color" id="color" onChange="PlanChange();" onClick="PlanChange();"><option value="0">Нет</option><option value="1">Да (+'.number_format($youtube_cena_color, 4,".","").' руб./визит)</option></select></div><div id="bl31" style="text-align:left;"></div></td>';
			...
			echo '<tr>';
				...
				echo '<td style="height:31px;"><div id="bl5"><select name="active" id="active" onChange="PlanChange();" onClick="PlanChange();"><option value="0">Нет</option><option value="1">Да (+'.number_format(($youtube_cena_active * (100+$youtube_nacenka)/100), 4,".","").' руб./визит)</option></select></div><div id="bl51" style="text-align:left;"></div></td>';
			...
			echo '<tr>';
				...
				echo '<td style="height:31px;"><div id="bl6">';
					echo '<select name="revisit" id="revisit" onChange="PlanChange();" onClick="PlanChange();">';
						...
						echo '<option value="1">Каждые 48 часов (+ '.number_format($youtube_cena_revisit_1, 4,".","").' руб./визит)</option>';
						echo '<option value="2">1 раз (+ '.number_format($youtube_cena_revisit_2, 4,".","").' руб./визит)</option>';
					echo '</select>';
			...
			echo '<tr id="type_serf_r">';
				...
				echo '<td style="height:31px;"><div id="bl8">';
					echo '<select name="unic_ip" id="unic_ip" onChange="PlanChange();" onClick="PlanChange();">';
						echo '<option value="0">Нет</option>';
						echo '<option value="1">Да (100% совпадение) &mdash; (+ '.number_format($youtube_cena_unic_ip_1, 4,".","").' руб./визит)</option>';
						echo '<option value="2">Усиленный по маске до 2 чисел (255.255.X.X) &mdash; (+ '.number_format($youtube_cena_unic_ip_2, 4,".","").' руб./визит)</option>';
					echo '</select>';
				echo '</div><div id="bl81" style="text-align:left;"></div>';
				echo '</td>';
	...
	//echo '<span id="adv-title3" class="adv-title-open" onclick="ShowHideBlock(3);">Способ оплаты</span>';
///	echo '<div id="adv-block3" style="display:block;">';
		...
		echo '<tbody>';
			...
			//echo '<tr>';
				...
				//echo '<td align="left">';
					//echo '<select id="method_pay" name="method_pay" style="margin-bottom:1px;">';
