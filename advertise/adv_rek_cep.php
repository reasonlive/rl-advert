...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = $mysqli->query("SELECT `money` FROM `tb_ads_rc` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if($sql_id->num_rows>0) {
			...
			if($money_user>=$money_pay) {
				...
				$mysqli->query("UPDATE `tb_ads_rc` SET `status`='1', `date`='".time()."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
				$mysqli->query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: - Рекламная цепочка','Списано','rashod')") or die($mysqli->error);
...
if(count($_POST)>0) {
	...
	$sql_bl = $mysqli->query("SELECT * FROM `tb_black_sites` WHERE `domen` LIKE '%$black_url%'");
...
$system_pay[21] = "AdvCash";
		...
		$color_to[0]="НЕТ";
		$color_to[1]="ДА (+ $cena_color_rek_cep руб.)";

		$summa = round(($cena_rek_cep + ($cena_color_rek_cep * $color)),2);
		/* $summa = number_format(($summa * (100-$cab_skidka)/100),2,".",""); */

		$mysqli->query("DELETE FROM `tb_ads_rc` WHERE `status`='0' AND `date`<'".(time()-(24*60*60))."'") or die($mysqli->error);
		...
		$check_wmid = $mysqli->query("SELECT `id` FROM `tb_ads_rc` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_rc` SET `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`date`='".time()."',`wmid`='$wmid_user',`username`='$username',`color`='$color',`url`='$url',`description`='$description',`ip`='$laip',`money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_rc` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`date`,`wmid`,`username`,`color`,`url`,`description`,`ip`,`money`) 
		...
		$sql_id = $mysqli->query("SELECT `id` FROM `tb_ads_rc` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		...
		$shp_item = "12";
		$inv_desc = "Оплата рекламы: рекламная цепочка, счет:$merch_tran_id";
		$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
		$inv_desc_en = "Pay advertise: reklamnaya cepochka, order:$merch_tran_id";
	...
	function obsch(){
		...
		var cena = <?php echo $cena_rek_cep;?>;
		var cena_color = <?php echo $cena_color_rek_cep;?>;
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">рекламная цепочка - что это?</span>';
		echo '<div id="adv-block-info" style="display:block; padding:5px 7px 10px 7px; text-align:justify; background-color:#FFFFFF;">';
		echo 'Это ссылки, размещаемые на всех страницах  по 5 ссылок и смещаются вправо по мере добавления новых ссылок. Преимущество этого вида рекламы в том, что по вашей ссылке может быть неограниченное колличество переходов!';
	echo '</div>';
	echo '<br><br>';
		...
		echo '<tr>';
			...
			echo '<td>';
				echo '<select name="color" id="color" onChange="obsch();" onClick="obsch();">';
					...
					echo '<option value="1">Да (+'.number_format($cena_color_rek_cep,2,".","'").' руб.)</option>';
				echo '</select>';
			echo '</td>';
		echo '</tr>';
		echo '<tr>';
