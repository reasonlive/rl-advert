...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	$sql_user = $mysqli->query("SELECT `wmid`,`wm_purse`,`money_rb`,`test_drive`,`reiting` FROM `tb_users` WHERE `username`='$username'");
	if($sql_user->num_rows>0) {
		$row_user = $sql_user->fetch_row();
		$wmid_user = $row_user["0"];
		$wmr_user = $row_user["1"];
		$money_user = $row_user["2"];
		$test_drive = $row_user["3"];
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
		...
		$sql_id = $mysqli->query("SELECT `id` FROM `tb_ads_dlink` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if($sql_id->num_rows>0) {

			$mysqli->query("UPDATE `tb_users` SET `test_drive`='1' WHERE `username`='$username'") or die($mysqli->error);
...
if(count($_POST)>0) {
	...
	if($sql_bl->num_rows>0 && $black_url!=false) {
		...
		if($sql_check->num_rows>0) {
			...
			VALUES('0','".session_id()."','$merch_tran_id','$method_pay','$type_serf','".time()."','$wmid_user','$username','$country','$content','$active','$revisit','$color','$timer','$nolimitdate','$limit_d','$limit_h','$limit_d','$limit_h','$url','$title','$description','$plan','$plan','$laip','0')") or die($mysqli->error);
		}
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Динамические ссылки ТЕСТ-ДРАВ - что это?</span>';
		echo '<div id="adv-block-info" style="display:block; padding:5px 7px 10px 7px; text-align:justify; background-color:#FFFFFF;">';
		echo 'Это <span style="color:#FF0000;">Test Drive</span> - '.$plan.' просмотров вашего сайта бесплатно<br>';
		echo 'Ваша ссылка будет размещена на странице просмотра сайтов автоматически, и будет снята только после того количества показов, которое было заказано Вами (при условии соблюдения правил*).<br>';
		echo 'Функция <span style="color:#FF0000;">Test Drive</span> доступна зарегистрированным пользователям только 1 раз!!!<br>';
	echo '</div>';
	echo '<br><br>';
