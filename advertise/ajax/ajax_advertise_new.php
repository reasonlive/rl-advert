...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest") {
		...
		if($sql_user->num_rows > 0) {
			...
			if($sql_cab->num_rows > 0) {
				...
				if($row_cab["status"] == 1 && $user_money_rek >= $row_cab["start_sum"]) {
					...
					$cab_text = '<tr><td align="left">Ваша скидка рекламодателя:</td><td align="left"><b class="text-green">'.$cab_skidka.'</b> %</td></tr>';
				}
			}
			$sql_cab->free();
		}else{
	...
	if($type_ads == "pay_visits") {
		...
		$result_text = "ERROR"; $message_text = "$result_text: Тип рекламы не определен";
		exit(my_json_encode($result_text, $message_text));
	}
}else{
