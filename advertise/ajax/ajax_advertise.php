...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest") {
	...
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
		$sql_user = $mysqli->query("SELECT `id`,`username`,`wmid`,`wm_purse`,`money_rb`,`money_rek`,`referer`,`referer2`,`referer3` FROM `tb_users` WHERE `username`='$user_name' AND md5(`password`)='$user_pass'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
		if($sql_user->num_rows>0) {
			...
			$sql_cab = $mysqli->query("SELECT * FROM `tb_cabinet` WHERE `id`='1'") or die(my_json_encode($ajax_json, "ERROR", $mysqli->error));
