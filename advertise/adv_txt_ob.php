...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"]) && isset($my_referer)) {
		if($my_referer!=false) {
			$sql_p = $mysqli->query("SELECT `p_txt`,`discount_partner` FROM `tb_users_partner` WHERE `username`='$my_referer' AND `discount_partner`>'0'");
			if($sql_p->num_rows>0) {
				$row_p = $sql_p->fetch_array();
				$p_txt_ref = $row_p["p_txt"];
				$discount_partner = $row_p["discount_partner"];
				$my_discount = p_floor(($p_txt_ref * $discount_partner)/100, 2);
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = $mysqli->query("SELECT * FROM `tb_ads_txt` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if($sql_id->num_rows>0) {
			...
			if($money_user>=$money_pay) {
				...
				$mysqli->query("UPDATE `tb_ads_txt` SET `status`='1', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
				...
				PartnerSet($username, "p_txt", $start_cena, $plan, $type_banner=false);
				...
				$add_percent_user = floor($p_txt + $add_per);
				if($add_percent_user>$partner_max_percent) $add_percent_user = $partner_max_percent;
				$mysqli->query("UPDATE `tb_users` SET `p_txt`='$add_percent_user' WHERE `username`='$username'") or die($mysqli->error);

				if($my_referer!=false) {
					$sql_up = $mysqli->query("SELECT `p_txt` FROM `tb_users` WHERE `username`='$my_referer' AND `p_txt`>'0'");
					if($sql_up->num_rows>0) {
						$row_up = $sql_up->fetch_row();
						$ref_p_txt = $row_up["0"];

						$money_add_referer = floatval($money_pay / 100 * $ref_p_txt);
						$mysqli->query("UPDATE `tb_users` SET `money`=`money`+'$money_add_referer' WHERE `username`='$my_referer'") or die($mysqli->error);

						$mysqli->query("INSERT INTO `tb_partner` (`time`, `username`, `referer`, `type`, `money`, `percent`) VALUES('".time()."','$username','$my_referer','p_txt','$money_add_referer', '$ref_p_txt')") or die("Error".$mysqli->error);
					}
				}
...
if(count($_POST)>0) {
	...
	}elseif($plan<$min_txt_ob) {
		echo '<span class="msg-error">Минимальный заказ - '.$min_txt_ob.' (дней).</span><br>';
...
$system_pay[21] = "AdvCash";
		...
		$summa = round($plan*$cena_txt_ob,2);
		/* $summa = number_format(($summa * (100-$cab_skidka)/100),2,".",""); */

		$mysqli->query("DELETE FROM `tb_ads_txt` WHERE `status`='0' AND `date`<'".(time()-(24*60*60))."'") or die($mysqli->error);
		...
		$check_wmid = $mysqli->query("SELECT `id` FROM `tb_ads_txt` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_txt` SET `merch_tran_id`='$merch_tran_id', `method_pay`='$method_pay', `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `ip`='$laip', `money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_txt` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`date`,`date_end`,`plan`,`url`,`description`,`ip`,`money`) 
		...
		$sql_id = $mysqli->query("SELECT `id` FROM `tb_ads_txt` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		...
		echo '<table class="tables">';
			...
			echo '<tr><td><b>Количество дней:</b></td><td>'.$plan.' ('.number_format(($plan * $cena_txt_ob),2,".","'").' руб.)</td></tr>';
			echo "$cab_text";
		...
		$shp_item = "9";
		$inv_desc = "Оплата рекламы: текстовое объявление, план:$plan, счет:$merch_tran_id";
		$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
		$inv_desc_en = "Pay advertise: textovoe obyavlenie, plan:$plan, order:$merch_tran_id";
	...
	function SbmFormB() {
		...
		for (var i=0;i<arrayElem.length;i++){
			...
			if ((document.forms["formzakaz"].plan.value == '')|(document.forms["formzakaz"].plan.value < <?=$min_txt_ob;?> )) {
				alert('Мнимальное количество дней - <?=$min_txt_ob;?>');
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Текстовые объявления - что это?</span>';
		echo '<div id="adv-block-info" style="display:block; padding:5px 7px 10px 7px; text-align:justify; background-color:#FFFFFF;">';
		echo 'это ссылки, которые размещаются на определенное время, и удаляются только после завершения выбранного времени. При этом количество переходов может быть неограниченное.<br />';
		echo '<b>Какой от этого эффект?</b> - В первую очередь Вы получаете целевую аудиторию на свой проект. Также стоит отметить, что ссылка размещается без редиректов, в связи с чем возможно также повышение тИЦ и PR. В отличие от обычных ссылок, текстовое объявление может занимать до 255 символов.<br /><br />';
		echo 'Стоимость размещения: <b>'.$cena_txt_ob.'</b> руб/сутки<br>';
	echo '</div>';
	echo '<br><br>';
	...
	echo '<tbody>';
		...
		echo '<tr>';
			...
			echo '<td><input type="text" name="plan" id="plan" maxlength="7" value="'.$min_txt_ob.'" class="ok12" style="text-align:center;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;(минимум '.$min_txt_ob.')</td>';
		echo '</tr>';
		echo '<tr>';
