...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"]) && isset($my_referer)) {
		if($my_referer!=false) {
			$sql_p = $mysqli->query("SELECT `p_psd`,`discount_partner` FROM `tb_users_partner` WHERE `username`='$my_referer' AND `discount_partner`>'0'");
			if($sql_p->num_rows>0) {
				$row_p = $sql_p->fetch_array();
				$p_psd_ref = $row_p["p_psd"];
				$discount_partner = $row_p["discount_partner"];
				$my_discount = p_floor(($p_psd_ref * $discount_partner)/100, 2);
	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if($sql_id->num_rows>0) {
			...
			if($money_user>=$money_pay) {
				...
				$mysqli->query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: - Псевдо-динамические ссылки $plan дн.','Списано','rashod')") or die($mysqli->error);

				stat_pay('psevdo', $money_pay);
				...
				PartnerSet($username, "p_psd", $start_cena, $plan, $type_banner=false);
				...
				$add_percent_user = floor($p_psd + $add_per);
				if($add_percent_user>$partner_max_percent) $add_percent_user = $partner_max_percent;
				$mysqli->query("UPDATE `tb_users` SET `p_psd`='$add_percent_user' WHERE `username`='$username'") or die($mysqli->error);

				if($my_referer!=false) {
					$sql_up = $mysqli->query("SELECT `p_psd` FROM `tb_users` WHERE `username`='$my_referer' AND `p_psd`>'0'");
					if($sql_up->num_rows>0) {
						$row_up = $sql_up->fetch_row();
						$ref_p_psd = $row_up["0"];

						$money_add_referer = floatval($money_pay / 100 * $ref_p_psd);
						$mysqli->query("UPDATE `tb_users` SET `money`=`money`+'$money_add_referer' WHERE `username`='$my_referer'") or die($mysqli->error);

						$mysqli->query("INSERT INTO `tb_partner` (`time`, `username`, `referer`, `type`, `money`, `percent`) VALUES('".time()."','$username','$my_referer','p_psd','$money_add_referer', '$ref_p_psd')") or die("Error".$mysqli->error);
					}
				}
...
if(count($_POST)>0) {
	...
	}elseif($plan==false | $plan<$min_psevdo) {
		echo '<span class="msg-error">Минимальный заказ - '.$min_psevdo.' показов.</span><br>';
...
$system_pay[21] = "AdvCash";
		...
		$color_to[1]="ДА (".number_format(($plan * $color * $cena_color_psevdo),2,".","'")." руб.)";


		$summa = $plan * ($cena_psevdo + $color * $cena_color_psevdo);
		$summa = round($summa,2);
		...
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_psevdo` SET `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`wmid`='$wmid_user',`username`='$username',`plan`='$plan',`date`='".time()."',`date_end`='".(time() + $plan*24*60*60)."',`color`='$color',`url`='$url',`description`='$description',`ip`='$laip',`money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_psevdo` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`plan`,`date`,`date_end`,`color`,`url`,`description`,`ip`,`money`) 
			VALUES ('0','".session_id()."','$merch_tran_id','$method_pay','$wmid_user','$username','$plan','".time()."','".(time() + $plan*24*60*60)."','$color','$url','$description','$laip','$summa')") or die($mysqli->error);
		}
		...
		echo '<table class="tables">';
			...
			echo '<tr><td><b>Количество дней:</b></td><td>'.$plan.' ('.number_format(($plan * $cena_psevdo),2,".","'").' руб.)</td></tr>';
		...
		$shp_item = "3";
		$inv_desc = "Оплата рекламы: Псевдо-динамическая ссылка, план:$plan, счет:$merch_tran_id";
		$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
		$inv_desc_en = "Pay advertise: psevdo-dinamic link, plan:$plan, order:$merch_tran_id";
	...
	function SbmFormB() {
		...
		for (var i=0;i<arrayElem.length;i++){
			...
			if ((document.forms["formzakaz"].plan.value == '') | (document.forms["formzakaz"].plan.value < <?=$min_psevdo;?>) ) {
				alert('Мнимальное количество дней <?=$min_psevdo;?>');
	...
	function obsch(){
		...
		var cena_psevdo = <?php echo $cena_psevdo;?>;
		var cena_color = <?php echo $cena_color_psevdo;?>;

		var price = plan * (cena_psevdo + color * cena_color);
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Псевдо-динамические ссылки</span>';
		echo '<div id="adv-block-info" style="display:block; padding:5px 7px 10px 7px; text-align:justify; background-color:#FFFFFF;">';
		echo 'Размещаются на определенное количество дней. При этом количество переходов по ним может быть неограниченное. Вы получаете именно заинтересованных посетителей на свой проект. Ссылка размещается на странице - "Серфинг сайтов".';
	echo '</div>';
	echo '<br><br>';
	...
	echo '<tbody>';
		...
		echo '<tr>';
			...
			echo '<td><input type="text" name="plan" id="plan" maxlength="7" value="1" class="ok12" style="text-align:center;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;&nbsp;(минимум '.$min_psevdo.')</td>';
		echo '</tr>';
		echo '<tr>';
			...
			echo '<td>';
				echo '<select name="color" id="color" onChange="obsch();" onClick="obsch();">';
					...
					echo '<option value="1">Да (+'.number_format($cena_color_psevdo,2,".","'").' руб./сутки)</option>';
				echo '</select>';
			echo '</td>';
		echo '</tr>';
		echo '<tr>';
