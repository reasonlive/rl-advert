...
$cena_kat = number_format($cena_kat,2,".","");
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = $mysqli->query("SELECT * FROM `tb_ads_kat` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if($sql_id->num_rows>0) {
			...
			if($money_user>=$money_pay) {
				...
				$mysqli->query("UPDATE `tb_ads_kat` SET `status`='1', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die($mysqli->error);
...
if(count($_POST)>0) {
	...
	}elseif($plan<$cena_kat_min) {
		echo '<span class="msg-error">Минимальный заказ - '.$cena_kat_min.' дней.</span><br>';
...
$system_pay[21] = "AdvCash";
		...
		$color_to[0]="Нет";
		$color_to[1]="Да (".number_format(($cena_kat_color * $plan),2,".","`")." руб.)";

		$summa = round(($plan * ($cena_kat + $cena_kat_color * $color)),2);
		$summa = number_format(($summa * (100-$cab_skidka)/100),2,".","");

		$mysqli->query("DELETE FROM `tb_ads_kat` WHERE `status`='0' AND `date`<'".(time()-(24*60*60))."'") or die($mysqli->error);
		...
		$check_wmid = $mysqli->query("SELECT `id` FROM `tb_ads_kat` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		if($check_wmid->num_rows>0) {
			$mysqli->query("UPDATE `tb_ads_kat` SET `merch_tran_id`='$merch_tran_id', `method_pay`='$method_pay', `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `color`='$color', `ip`='$laip', `money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die($mysqli->error);
		}else{
			$mysqli->query("INSERT INTO `tb_ads_kat` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`date`,`date_end`,`plan`,`url`,`description`,`color`,`ip`,`money`) 
		...
		$sql_id = $mysqli->query("SELECT `id` FROM `tb_ads_kat` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		...
		echo '<table class="tables">';
			...
			echo '<tr><td><b>Количество дней:</b></td><td>'.$plan.' ('.number_format(($plan * $cena_kat),2,".","'").' руб.)</td></tr>';
		...
		$shp_item = "6";
		$inv_desc = "Оплата рекламы: Каталог индексируемых ссылок, план:$plan, счет:$merch_tran_id";
		$inv_desc_utf8 = iconv("CP1251", "UTF-8", $inv_desc);
		$inv_desc_en = "Pay advertise: Catalog of indexed links, plan:$plan, order:$merch_tran_id";
	...
	function SbmFormB() {
		...
		for (var i=0;i<arrayElem.length;i++){
			...
			if ((document.forms["formzakaz"].plan.value == '') | (document.forms["formzakaz"].plan.value < <?=@$cena_kat_min;?>) ) {
				alert('Минимальное количество дней <?=@$cena_kat_min;?>');
	...
	function _obsch(){
		...
		var cena_p = <?php echo $cena_kat;?>;
		var cena_color = <?php echo $cena_kat_color;?>;
		var price = (color * cena_color + cena_p) * plan;
		gebi('pricet').innerHTML = 'Стоимость заказа:';
		gebi('prices').innerHTML = '<b style="color:#FF0000;">' + price.toFixed(2) + ' руб.</b>';
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		echo '<span id="adv-title-info" class="adv-title-open" onclick="ShowHideBlock(\'-info\');" style="border-top:solid 1px #46A2FF; text-align:left; padding-left:50px; margin-bottom:2px;">Каталог индексируемых ссылок - что это?</span>';
		echo '<div id="adv-block-info" style="display:block; padding:5px 7px 10px 7px; text-align:justify; background-color:#FFFFFF;">';
			echo 'Основой продвижения вашего сайта в поисковых системах являются важные показатели <b>ИКС</b> (Яндекс) и <b>PageRank</b> (Google). ';
		...
		echo 'На данный момент показатели <b style="color:#3A5FCD">'.$_SERVER["HTTP_HOST"].'</b> ИКС = ';
		echo '<div style="display: block; margin: 15px auto 3px auto; text-align:center;"><a href="https://yandex.ru/cy?base=0&amp;host=supreme-garden.ru" target="_blank" title="Индекс качества сайта" ><img src="https://www.yandex.ru/cycounter?supreme-garden.ru" width="88" height="31" alt="Индекс качества сайта" border="0" /></a></div>';
		echo '</div>';
	...
	echo '<tbody>';
		...
		echo '<tr>';
			...
			echo '<td><input type="text" name="plan" id="plan" maxlength="7" value="7" class="ok12" style="text-align:center;" onChange="_obsch();" onKeyUp="_obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;(минимум '.@$cena_kat_min.')</td>';
		echo '</tr>';
		echo '<tr>';
			...
			echo '<td>';
				echo '<select name="color" id="color" onChange="_obsch();" onClick="_obsch();">';
					...
					echo '<option value="1">Да (+'.number_format($cena_kat_color,2,".","'").' руб./сутки)</option>';
				echo '</select>';
			echo '</td>';
		echo '</tr>';
		echo '</tbody>';
	...
	echo '<tbody>';
                echo '<tr>';
		echo '<td>Стоимость показа в сутки:</td>';
		echo '<td><b><span style="color:#228B22;">'.$cena_kat.'</span></b></td>';
	echo '</tr>';
		echo '<tr>';
