...
require($_SERVER["DOCUMENT_ROOT"]."/merchant/robokassa/rk_config.php");
define('PATH_TO_LOG', dirname(__FILE__).'/');

$merch_amount = (isset($_REQUEST["OutSum"])) ? $_REQUEST["OutSum"] : false;
$merch_tran_id = (isset($_REQUEST["InvId"])) ? intval(trim($_REQUEST["InvId"])) : false;
...
$crc = (isset($_REQUEST["SignatureValue"])) ? strtoupper($_REQUEST["SignatureValue"]) : false;
$my_crc = strtoupper(md5("$merch_amount:$merch_tran_id:$mrh_pass2:Shp_item=$shp_item"));

#запись в файл информации о прведенной операции
$f=@fopen(PATH_TO_LOG."rk_order.txt","a+") or die("error");
@fputs($f,"order_num: $merch_tran_id; SHP_ITEM: $shp_item; Summ: $merch_amount; Date: ".DATE("d.m.Y H:i")."\n");
fclose($f);
		...
		if($sql->num_rows) {
			...
			/////////////////////	
				if($bon_pay_status==1){
					...
					VALUES('$username', '$merch_amount1', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через ROBOKASSA','Зачислено', 'popoln')") or die($mysqli->error);
		...
		/////////////////////////////////////////////////////
			...
			/////////////////////	
				if($bon_inv_status==1){
					...
					$mysqli->query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
					VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через RoboKassa','Зачислено', 'popoln')") or die($mysqli->error);
