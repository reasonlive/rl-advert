...
require($_SERVER["DOCUMENT_ROOT"]."/merchant/freekassa/fk_config.php");
define('PATH_TO_LOG', dirname(__FILE__).'/');

$MERCHANT_ID = ( isset($_REQUEST["MERCHANT_ID"]) && preg_match("|^[\d]{1,11}$|", htmlspecialchars(trim($_REQUEST["MERCHANT_ID"]))) ) ? htmlspecialchars(trim($_REQUEST["MERCHANT_ID"])) : false;
$AMOUNT = (isset($_REQUEST["AMOUNT"])) ? htmlspecialchars($_REQUEST["AMOUNT"]) : false;
$FK_ORDER_ID = (isset($_REQUEST["intid"])) ? htmlspecialchars($_REQUEST["intid"]) : false;
$MERCHANT_ORDER_ID = (isset($_REQUEST["MERCHANT_ORDER_ID"])) ? htmlspecialchars($_REQUEST["MERCHANT_ORDER_ID"]) : false;
$CUR_ID = ( isset($_REQUEST["CUR_ID"]) && preg_match("|^[\d]{1,11}$|", htmlspecialchars(trim($_REQUEST["CUR_ID"]))) ) ? htmlspecialchars(trim($_REQUEST["CUR_ID"])) : false;
$SIGN = (isset($_REQUEST["SIGN"]) && preg_match("|^[0-9a-fA-F]{32}$|", htmlspecialchars(trim($_REQUEST["SIGN"])))) ? strtoupper(htmlspecialchars($_REQUEST["SIGN"])) : false;
$MY_SIGN = strtoupper(md5(join(':', array($MERCHANT_ID, $AMOUNT, $fk_secretkey_2, $MERCHANT_ORDER_ID))));

if($MERCHANT_ORDER_ID != false) {
	$exp = explode(":", $MERCHANT_ORDER_ID);
...
$merch_amount = number_format($AMOUNT, 2, ".", "");

#запись в файл информации о проведенной операции
$f = @fopen(PATH_TO_LOG."fk_order.txt","a+") or die("error");
@fputs($f,"AMOUNT: $merch_amount; SHP_ITEM: $shp_item; ORDER_ID: $merch_tran_id; FK_ORDER_ID: $FK_ORDER_ID; CUR_ID: $CUR_ID; DATE: ".DATE("d.m.Y H:i")."; SIGN: $SIGN; MY_SIGN: $MY_SIGN;\n");
@fclose($f);
#запись в файл информации о проведенной операции
		...
		if($sql->num_rows) {
			...
			/////////////////////	
				if($bon_pay_status==1){
					...
					$mysqli->query("UPDATE `tb_add_pay` SET `status`='1', `date`='".time()."' WHERE `merch_tran_id`='$merch_tran_id' AND `username`='$username'") or die($mysqli->error);

					$mysqli->query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
					VALUES('$username', '$merch_amount1', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через FreeKassa','Зачислено', 'popoln')") or die($mysqli->error);
		...
		/////////////////////////////////////////////////////
			...
			/////////////////////	
				if($bon_inv_status==1){
					...
					$mysqli->query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
					VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через FreeKassa','Зачислено', 'popoln')") or die($mysqli->error);
